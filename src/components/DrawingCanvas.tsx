...
const DrawingCanvas: React.FC<DrawingCanvasProps> = ({ onSave, prompt }) => {
  ...
  const handleClearCanvas = () => {
    ...
    if (bgLayer && bgLayer.context) {
      bgLayer.context.fillStyle = 'white';
      bgLayer.context.fillRect(
        0, 0, 
    ...
    const mainLayer = layerManagerRef.current.getLayer('main');
    if (mainLayer && mainLayer.context) {
      mainLayer.context.clearRect(
        0, 0, 
        canvasRef.current?.width ?? 500, 
        canvasRef.current?.height ?? 500
      );
