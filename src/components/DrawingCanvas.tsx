...
const DrawingCanvas: React.FC<DrawingCanvasProps> = ({ onSave, prompt }) => {
  ...
  return (
    <div className="bg-white rounded-lg shadow p-4 canvas-container">
      {/* Drawing tools */}
      <div className="mb-4 flex flex-wrap items-center gap-2">
        <ToggleGroup type="single" value={tool} onValueChange={(value) => value && setTool(value as any)}>
          <ToggleGroupItem value="pen" aria-label="Pen tool" className="p-2">
            <Pen className="h-4 w-4" />
          </ToggleGroupItem>
          <ToggleGroupItem value="eraser" aria-label="Eraser tool" className="p-2">
            <Eraser className="h-4 w-4" />
          </ToggleGroupItem>
          <ToggleGroupItem value="text" aria-label="Text tool" className="p-2">
            <Type className="h-4 w-4" />
          </ToggleGroupItem>
          <ToggleGroupItem value="rectangle" aria-label="Rectangle tool" className="p-2">
            <Square className="h-4 w-4" />
          </ToggleGroupItem>
          <ToggleGroupItem value="circle" aria-label="Circle tool" className="p-2">
          ...
          <Popover open={showThemeSelector} onOpenChange={setShowThemeSelector}>
            ...
            <PopoverContent className="w-80">
              <div className="space-y-4">
                <h3 className="font-medium">Visual Theme</h3>
                <Select 
                ...
                <h3 className="font-medium mt-4">Seasonal Theme</h3>
                <Select 
      ...
      {/* Text editing context menu for selected text */}
      {selectedTextIndex !== null && (
        <div className="mb-4 p-2 bg-gray-100 rounded-md flex gap-2">
          <Button size="sm" variant="outline" onClick={handleEditText}>
            Edit Text
          </Button>
          <Button size="sm" variant="outline" className="text-red-500" onClick={handleRemoveText}>
            Remove Text
          </Button>
        </div>
      )}
      ...
      {/* Canvas */}
      <div className="relative mb-4 bg-white rounded border border-gray-200 overflow-hidden">
        {prompt && (
          <div className="p-2 bg-yellow-50 border-b border-yellow-100 text-sm">
            <strong>Today's prompt:</strong> {prompt}
          </div>
        )}
        <canvas
          ref={canvasRef}
