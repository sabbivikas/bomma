...
const DrawingCanvas: React.FC<DrawingCanvasProps> = ({ onSave, prompt }) => {
  ...
  const ThemePreview = () => {
    ...
    return (
      <div className="p-2 rounded-md" style={{
        background: visualThemeConfig?.backgroundStyle ? visualThemeConfig.backgroundStyle.replace('bg-', '') : 'white'
      }}>
        <div className="text-center">
          <div className="font-medium">{visualThemeConfig?.name || 'Default'}</div>
          {seasonalThemeConfig && seasonalThemeConfig.id !== 'none' && (
            <div className="text-xs opacity-75">+ {seasonalThemeConfig.name}</div>
          )}
        </div>
      </div>
    );
  ...
  // Render the text edit controls when a text is selected
  const renderTextControls = () => {
    if (selectedTextIndex === null) return null;
    
    return (
      <div className="absolute right-2 top-2 bg-white shadow-md rounded-md p-2 flex gap-2 z-20">
        <Button 
          size="sm" 
          variant="outline" 
  ...
  // Render the main canvas component
  return (
    <div className="relative">
      {/* Text controls for selected text */}
      {selectedTextIndex !== null && renderTextControls()}
      
      {/* Font and text controls when text tool is selected */}
      {tool === 'text' && (
        <div className="mb-4 p-3 bg-slate-50 rounded-md border">
          ...
          <div className="mt-2 text-sm text-blue-600">
            Tap canvas to add text, or tap existing text to edit/move
          </div>
        </div>
      )}
      ...
      <div className="flex flex-wrap justify-between gap-2 mb-4">
          ...
          <Popover>
            ...
            <PopoverContent className="w-full">
              <div className="grid grid-cols-5 gap-2">
                {["#000000", "#FF0000", "#00FF00", "#0000FF", "#FFFF00", "#FF00FF", "#00FFFF", "#FF9900", "#9900FF", "#990000"].map((colorOption) => (
                  <div
                    key={colorOption}
                    className={`w-8 h-8 rounded-md cursor-pointer ${color === colorOption ? 'ring-2 ring-blue-500' : ''}`}
        ...
        <div className="flex gap-1">
          ...
          >
            ...
            {isPublishing ? "Publishing..." : "Publish to Frame"}
          </Button>
        </div>
      </div>
      ...
      <div className="relative border rounded-md overflow-hidden">
        {/* Prompt indicator if provided */}
        {prompt && (
          <div className="absolute top-0 left-0 right-0 bg-blue-500 text-white p-2 z-10 opacity-75">
            {prompt}
          </div>
        )}
        ...
        <Button
          ...
          className="absolute bottom-2 right-2 text-xs h-7 opacity-50 hover:opacity-100 z-10"
        >
        ...
        {showThemeSelector && (
          <div className="absolute bottom-10 right-2 bg-white rounded-md shadow-lg p-4 z-20 w-64">
            <h4 className="text-sm font-medium mb-2">Visual Theme</h4>
            <div className="grid grid-cols-3 gap-2 mb-4">
              {visualThemes.map((vTheme) => (
                <div
                  key={vTheme.id}
                  className={`p-2 rounded-md cursor-pointer transition-all ${theme.visualTheme === vTheme.id ? 'ring-2 ring-blue-500' : 'hover:bg-gray-50'}`}
                  onClick={() => setVisualTheme(vTheme.id as VisualTheme)}
                >
                  <div className="text-center text-xs truncate">{vTheme.name}</div>
                </div>
              ))}
            </div>
            
            <h4 className="text-sm font-medium mb-2">Seasonal Overlay</h4>
            <div className="grid grid-cols-3 gap-2">
              {seasonalThemes.map((sTheme) => (
                <div
                  key={sTheme.id}
                  className={`p-2 rounded-md cursor-pointer transition-all ${theme.seasonalTheme === sTheme.id ? 'ring-2 ring-blue-500' : 'hover:bg-gray-50'}`}
                  onClick={() => setSeasonalTheme(sTheme.id as SeasonalTheme)}
                >
                  <div className="text-center text-xs truncate">{sTheme.name}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <DialogContent>
          ...
          <div className="space-y-4 py-4">
            <div className="flex items-center gap-4">
              <Label htmlFor="textColor" className="w-20">Color</Label>
              <div 
                className="w-8 h-8 rounded-md cursor-pointer"
                style={{ backgroundColor: color }}
                onClick={() => {
                  // Show color picker - simplified for the example
                  const newColor = window.prompt('Enter color (e.g. #FF0000)', color);
            ...
            <div className="flex items-center gap-4">
              <Label htmlFor="textSize" className="w-20">Size</Label>
              <Slider 
                id="textSize"
                value={textSize} 
                min={8} 
                ...
                className="w-40"
              />
              <span className="text-sm ml-2">{textSize[0]}px</span>
            </div>
            
            <div className="flex items-center gap-4">
              <Label htmlFor="textFont" className="w-20">Font</Label>
              <Select value={textFont} onValueChange={setTextFont}>
                <SelectTrigger className="w-40" id="textFont">
