...
export async function generateSampleDoodles(): Promise<Doodle[]> {
  ...
  // Enhanced cartoon-style doodle images - smaller set of complete strings
  const sampleImages = [
  ...
  // Use a counter to track successful creations
  let successCount = 0;

  // Insert sample doodles one by one
  for (const imageUrl of sampleImages) {
    const sampleDoodle = {
      image_url: imageUrl,
      prompt: "Sample cartoon doodle",
  ...
  // Fetch all doodles after creating samples
  return getAllDoodles();
}
